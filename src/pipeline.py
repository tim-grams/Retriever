...
from src.features.generator import create_tfidf_embeddings, create_all, create_BM2_feature, create_tfidf_feature, \
    create_glove_embeddings, create_glove_feature, create_elmo_embeddings, create_elmo_feature, create_jaccard_feature, create_POS_features, \
...
class Pipeline(object):
    ...
    def create_elmo_embeddings(self):
        assert self.collection['preprocessed'] is not None, "Preprocess the data first"

        elmo, self.collection = create_elmo_embeddings(self.collection, name='collection')
        elmo, self.queries = create_elmo_embeddings(self.queries, elmo=elmo, name='query')
        elmo, self.queries_test = create_elmo_embeddings(self.queries_test, elmo=elmo, name='query_test')
    ...
    def create_elmo_feature(self, path_collection: str = 'data/embeddings/elmo_collection_embeddings.pkl',
                             path_query: str = 'data/embeddings/elmo_query_embeddings.pkl'):
        self.features = create_elmo_feature(self.features, self.collection, self.queries, path_collection, path_query)
